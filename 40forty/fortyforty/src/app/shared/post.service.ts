...
export class PostService {
  ...
  sendPost(body, file?)
  {
    ...
    if(file)
    {
      this.uploadServ.doUpload(file);
      mediaURL = `https://40forty.s3.amazonaws.com/${currentUser.username}/${file.name}`;
    }
    ...
    const payload = `{
      "text" : "${body}",
      "author" : ${JSON.stringify(currentUser)},
      "mediaURL" : "${mediaURL}"
    }`;

    console.log(payload);

    return this.httpcli.post<Post>('http://localhost:8080/40forty/posts/create-post', payload, options);
  }
